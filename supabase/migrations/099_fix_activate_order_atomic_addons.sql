-- ============================================================
-- 099: Fix activate_order_atomic order_addons INSERT
-- ============================================================
-- Migrations 067 → 079 → 098 rewrote the addon upsert block
-- using columns (addon_id, unit_price, subtotal) and JSON keys
-- (addonId, unitPrice, subtotal) that have never existed on the
-- order_addons table or in the TypeScript caller payload.
--
-- The order_addons table (003_core_entities.sql) uses:
--   addon_name, addon_price, addon_type, total_amount, store_id
--
-- The TypeScript caller (order-repo.ts) sends snake_case keys
