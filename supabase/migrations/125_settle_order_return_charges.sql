-- ============================================================
-- 125: Return charges at settlement
--
-- Problem: when a customer returns a vehicle with less fuel,
-- causes damage, or incurs any condition-based charge, staff
-- had no way to add that charge at the point of settlement.
-- The orders.return_charges column existed since migration 003
-- but was never wired up in the API or UI.
--
-- Fix: settle_order_atomic now accepts
--   p_return_charges_delta numeric(12,2) DEFAULT 0
-- When > 0 we bump orders.final_total AND orders.return_charges
-- by the delta in the same UPDATE that stamps settled_at.
-- The application layer adds the delta to the balance-before-
-- deposit calculation so the deposit cascade (applied / refund)
-- and any remaining-to-collect amount all reflect the true
-- amount owed. The math stays balanced: final_total goes up
-- by the delta and the final settlement payment (or deposit
-- application) covers it, so finalBalanceDue resolves to zero.
--
-- Mirrors the pattern established in migration 093 for the
