-- Add deposits_closing_balance to cash_reconciliation so the deposit envelope
-- balance is carried forward day-over-day, mirroring closing_balance for the
-- main till.  Without this the expected envelope formula restarted from zero
-- each day, making expectedDepositsHeld negative on any day where prior-day
-- deposits were returned without new deposits being taken.

ALTER TABLE cash_reconciliation
  ADD COLUMN IF NOT EXISTS deposits_closing_balance numeric(12,2) NULL;

-- Update the atomic upsert RPC to persist the new column.
CREATE OR REPLACE FUNCTION reconcile_cash_atomic(
  ...
  p_deposits_closing_balance  numeric(12,2)
) RETURNS void
LANGUAGE plpgsql
AS $$
BEGIN
