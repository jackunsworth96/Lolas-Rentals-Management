-- ============================================================
-- 098: Add vehicle_name to activate_order_atomic order_items INSERT
-- ============================================================
-- The walk-in direct flow passes `vehicleName` in the p_order_items JSONB
-- but the RPC never read or persisted it, leaving order_items.vehicle_name
-- as NULL. The /orders/enriched endpoint derives the Vehicle column from
-- order_items.vehicle_name, so walk-in bookings always showed "—" for
-- vehicle and return datetime.
--
-- This migration also fixes the ON CONFLICT clause to update vehicle_id,
-- vehicle_name, pickup_datetime so those fields can be corrected on
-- re-activation.
-- ============================================================
...
BEGIN
  ...
  -- 2. Upsert order items — now includes vehicle_name so the enriched orders
...
-- Re-emit REVOKE/GRANT matching the typed signature (unchanged from 079).
