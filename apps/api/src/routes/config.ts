...
router.post('/payment-methods', edit, validateBody(z.object({
  id: z.string().min(1), name: z.string().min(1), isDepositEligible: z.boolean().optional(), isActive: z.boolean().optional(), surchargePercent: z.number().min(0).max(100).optional(), showOnCustomerWebsite: z.boolean().optional(), gatewayProvider: z.enum(['xendit']).nullable().optional(),
})), async (req, res, next) => {
...
router.put('/payment-methods/:id', edit, validateBody(z.object({
  name: z.string().min(1), isDepositEligible: z.boolean().optional(), isActive: z.boolean().optional(), surchargePercent: z.number().min(0).max(100).optional(), showOnCustomerWebsite: z.boolean().optional(), gatewayProvider: z.enum(['xendit']).nullable().optional(),
})), async (req, res, next) => {
