...
import { PeaceOfMindModal } from './PeaceOfMindModal.js';

/** Matches the Peace of Mind catalog add-on (name varies slightly by store). */
export function isPeaceOfMindAddonName(name: string): boolean {
  const n = name.toLowerCase();
  return n.includes('peace') || n.includes('mind');
}
...
function basketAssetIconSrc(name: string): string | null {
  ...
  const lower = name.toLowerCase();
  if (isPeaceOfMindAddonName(name)) return peaceOfMindIcon;
...
export function AddOnsSection({
  ...
  const [pomcModalOpen, setPomcModalOpen] = useState(false);
    ...
    <>
      <div className="divide-y divide-charcoal-brand/[0.08]">
        {standard.map((addon) => {
          ...
          const isPeaceOfMind = isPeaceOfMindAddonName(addon.name);
          ...
          return (
            <div key={id} className="first:pt-1 last:pb-1">
              <div
                ...
                className={`flex w-full items-center gap-3 rounded-lg py-3 text-left transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-teal-brand/40 ${
                  rowDisabled
                    ? 'cursor-not-allowed opacity-50'
                    : 'cursor-pointer hover:bg-sand-brand/30'
                }`}
                onClick={() => {
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-[13px] font-medium text-charcoal-brand">
                    {addon.name}
                    {isPeaceOfMind && (
                      <button
                        type="button"
                        onClick={(e) => {
                          e.stopPropagation();
                          setPomcModalOpen(true);
                        }}
                        className="ml-2 font-lato text-xs text-teal-brand underline underline-offset-2 hover:text-teal-brand/70"
                      >
                        What&apos;s included?
                      </button>
                    )}
      ...
      <PeaceOfMindModal open={pomcModalOpen} onClose={() => setPomcModalOpen(false)} />
    </>
  );
}
